...
public class ContentCachingResponseWrapper extends HttpServletResponseWrapper {
	...
	public String getResponseContent() {
		String characterEncoding = StringUtils.defaultIfEmpty(getCharacterEncoding(),"UTF-8");
		try {
			byte[] tmp=this.content.toByteArray();
			if(StringUtils.defaultString(getHeader("Content-Encoding")).equals("gzip")){
				tmp= RequestUtils.uncompress(tmp);
			}
			return new String(tmp, characterEncoding);
